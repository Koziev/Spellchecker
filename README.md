...
6. **Compound Word Handling**

Natural languages have open vocabularies—it's impossible to list all valid words definitively. One way new words emerge (e.g., nonce words or occasionalisms) is through compounding: combining two roots into a single word.

This spellchecker's language model includes compound word recognition, ensuring the `is_known_word` method avoids false negatives for valid compounds.
